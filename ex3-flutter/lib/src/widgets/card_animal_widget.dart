...
class CardAnimalWidget extends StatelessWidget {
  ...
  CardAnimalWidget(
      {this.image, required this.title, required this.id, this.type});
    ...
    return GestureDetector(
      onTap: () {
        Navigator.of(context)
            .pushNamed('animalsDetail', arguments: AnimalModel(id: id));
      },
      child: Container(
        width: MediaQuery.of(context).size.width,
        child: Column(
          children: <Widget>[
            _mostrarFoto(context),
            Container(
  ...
  Widget _mostrarFoto(BuildContext context) {
    if (image != null) {
      return FadeInImage(
